...
func UpdateenvironmentforrepositoryHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repositories/%s/%s/environments/%s/changes", cfg.BaseURL, workspace, repo_slug, environment_uuid)
...
func CreateUpdateenvironmentforrepositoryTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repositories_workspace_repo_slug_environments_environment_uuid_changes",
		mcp.WithDescription("Update an environment"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdateenvironmentforrepositoryHandler(cfg),
	}
}
