...
func GetenvironmentforrepositoryHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repositories/%s/%s/environments/%s", cfg.BaseURL, workspace, repo_slug, environment_uuid)
		...
		var result models.Deploymentenvironment
...
func CreateGetenvironmentforrepositoryTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repositories_workspace_repo_slug_environments_environment_uuid",
		mcp.WithDescription("Get an environment"),
		...
		mcp.WithString("environment_uuid", mcp.Required(), mcp.Description("The environment UUID.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetenvironmentforrepositoryHandler(cfg),
	}
}
