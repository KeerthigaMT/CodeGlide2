...
func GetrepositorypipelinecachecontenturiHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repositories/%s/%s/pipelines-config/caches/%s/content-uri", cfg.BaseURL, workspace, repo_slug, cache_uuid)
		...
		var result models.Pipelinecachecontenturi
...
func CreateGetrepositorypipelinecachecontenturiTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repositories_workspace_repo_slug_pipelines-config_caches_cache_uuid_content-uri",
		mcp.WithDescription("Get cache content URI"),
		...
		mcp.WithString("cache_uuid", mcp.Required(), mcp.Description("The UUID of the cache.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetrepositorypipelinecachecontenturiHandler(cfg),
	}
}
