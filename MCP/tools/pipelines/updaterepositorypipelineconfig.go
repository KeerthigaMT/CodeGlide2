...
func UpdaterepositorypipelineconfigHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.Pipelinesconfig
		...
		url := fmt.Sprintf("%s/repositories/%s/%s/pipelines_config", cfg.BaseURL, workspace, repo_slug)
		...
		var result models.Pipelinesconfig
...
func CreateUpdaterepositorypipelineconfigTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_repositories_workspace_repo_slug_pipelines_config",
		mcp.WithDescription("Update configuration"),
		...
		mcp.WithBoolean("enabled", mcp.Description("Input parameter: Whether Pipelines is enabled for the repository.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdaterepositorypipelineconfigHandler(cfg),
	}
}
