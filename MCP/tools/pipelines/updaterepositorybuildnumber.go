...
func UpdaterepositorybuildnumberHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.Pipelinebuildnumber
		...
		url := fmt.Sprintf("%s/repositories/%s/%s/pipelines_config/build_number", cfg.BaseURL, workspace, repo_slug)
		...
		var result models.Pipelinebuildnumber
...
func CreateUpdaterepositorybuildnumberTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_repositories_workspace_repo_slug_pipelines_config_build_number",
		mcp.WithDescription("Update the next build number"),
		...
		mcp.WithNumber("next", mcp.Description("Input parameter: The next number that will be used as build number.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdaterepositorybuildnumberHandler(cfg),
	}
}
