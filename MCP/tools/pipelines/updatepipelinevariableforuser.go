...
func UpdatepipelinevariableforuserHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUpdatepipelinevariableforuserTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_users_selected_user_pipelines_config_variables_variable_uuid",
		mcp.WithDescription("Update a variable for a user"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdatepipelinevariableforuserHandler(cfg),
	}
}
