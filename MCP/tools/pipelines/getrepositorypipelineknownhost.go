...
func GetrepositorypipelineknownhostHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetrepositorypipelineknownhostTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repositories_workspace_repo_slug_pipelines_config_ssh_known_hosts_known_host_uuid",
		mcp.WithDescription("Get a known host"),
		...
		mcp.WithString("known_host_uuid", mcp.Required(), mcp.Description("The UUID of the known host to retrieve.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetrepositorypipelineknownhostHandler(cfg),
	}
}
