...
func GetoidckeysHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/workspaces/%s/pipelines-config/identity/oidc/keys.json", cfg.BaseURL, workspace)
...
func CreateGetoidckeysTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_workspaces_workspace_pipelines-config_identity_oidc_keys.json",
		mcp.WithDescription("Get keys for OIDC in Pipelines"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetoidckeysHandler(cfg),
	}
}
