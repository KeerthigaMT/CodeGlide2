...
func DeleterepositorypipelinevariableHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeleterepositorypipelinevariableTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repositories_workspace_repo_slug_pipelines_config_variables_variable_uuid",
		mcp.WithDescription("Delete a variable for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DeleterepositorypipelinevariableHandler(cfg),
	}
}
