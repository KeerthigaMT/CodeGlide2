...
func DeleterepositorypipelineknownhostHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeleterepositorypipelineknownhostTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repositories_workspace_repo_slug_pipelines_config_ssh_known_hosts_known_host_uuid",
		mcp.WithDescription("Delete a known host"),
		...
		mcp.WithString("known_host_uuid", mcp.Required(), mcp.Description("The UUID of the known host to delete.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeleterepositorypipelineknownhostHandler(cfg),
	}
}
