...
func GetrepositorypipelinecachesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repositories/%s/%s/pipelines-config/caches", cfg.BaseURL, workspace, repo_slug)
		...
		var result models.Paginatedpipelinecaches
...
func CreateGetrepositorypipelinecachesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repositories_workspace_repo_slug_pipelines-config_caches",
		mcp.WithDescription("List caches"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetrepositorypipelinecachesHandler(cfg),
	}
}
