...
func Get_repositories_workspace_repo_slug_refsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repositories/%s/%s/refs%s", cfg.BaseURL, queryString)
		...
		var result models.Paginatedrefs
...
func CreateGet_repositories_workspace_repo_slug_refsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repositories_workspace_repo_slug_refs",
		mcp.WithDescription("List branches and tags"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_repositories_workspace_repo_slug_refsHandler(cfg),
	}
}
