...
func GetrepositoryhostedpropertyvalueHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repositories/%s/%s/properties/%s/%s", cfg.BaseURL, workspace, repo_slug, app_key, property_name)
		...
		var result models.Applicationproperty
...
func CreateGetrepositoryhostedpropertyvalueTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repositories_workspace_repo_slug_properties_app_key_property_name",
		mcp.WithDescription("Get a repository application property"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetrepositoryhostedpropertyvalueHandler(cfg),
	}
}
