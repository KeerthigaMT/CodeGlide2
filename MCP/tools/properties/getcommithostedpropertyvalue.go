...
func GetcommithostedpropertyvalueHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetcommithostedpropertyvalueTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repositories_workspace_repo_slug_commit_commit_properties_app_key_property_name",
		mcp.WithDescription("Get a commit application property"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetcommithostedpropertyvalueHandler(cfg),
	}
}
