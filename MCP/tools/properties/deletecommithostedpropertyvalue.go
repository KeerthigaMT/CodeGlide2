...
func DeletecommithostedpropertyvalueHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletecommithostedpropertyvalueTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repositories_workspace_repo_slug_commit_commit_properties_app_key_property_name",
		mcp.WithDescription("Delete a commit application property"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DeletecommithostedpropertyvalueHandler(cfg),
	}
}
