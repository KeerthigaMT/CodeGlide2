...
func SearchteamHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/teams/%s/search/code%s", cfg.BaseURL, username, queryString)
...
func CreateSearchteamTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_teams_username_search_code",
		mcp.WithDescription("Search for code in a team's repositories"),
		mcp.WithString("username", mcp.Required(), mcp.Description("The account to search in; either the username or the UUID in curly braces")),
